...
const schema = z.object({
  fromOrgId: z.string().min(1, "Source required"),
  toOrgId: z.string().min(1, "Destination required"),
...
export function CreateTransferModal({ open, onClose }: Props) {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<FormValues>({
    ...
    defaultValues: { bloodType: BloodType.O_POSITIVE, componentType: ComponentType.WHOLE_BLOOD, priority: "normal", quantity: 5 },
  });
  ...
  return (
    <FormModal open={open} onClose={onClose} title="New Transfer Request" description="Create a blood unit transfer between organizations.">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div><label className={lc}>Urgency</label><select {...register("priority")} className={ic}><option value="normal">Routine</option><option value="urgent">Urgent</option><option value="emergency">Emergency</option></select></div>
