...
export default function CommandCenterPage() {
  ...
  const alerts = Array.isArray(alertsData?.items || alertsData)
    ? alertsData?.items || alertsData
    : [];

  /**
   * ==================================================
   * TEMPORARY ETHIOPIAN BLOOD NETWORK MAP DATA
   * Replace with backend-driven logistics data later.
   * ==================================================
   */
  const logisticsNodes: MapMarkerData[] = [
    {
      id: 1,
      name: "Ethiopian National Blood Bank",
      ...
      status: "Stable National Reserve",
      inventory: 1240,
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-body-main text-m3-on-surface-variant mt-2">
            Real-time situational awareness across regional blood
            banks.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon="local_shipping"
          progress={{
            value: dashboard?.dispatchProgress ?? 65,
          ...
          <div className="flex-1 relative overflow-hidden">
              ...
              renderInfoWindow={(marker) => (
                <div className="min-w-[240px] text-black">
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    {marker.metadata &&
                      Object.entries(marker.metadata).map(
                        ([key, value]) => (
                          <div
                            key={key}
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-4">
                    <button className="py-2 rounded-lg bg-m3-secondary text-white text-xs font-medium hover:opacity-90 transition-opacity">
                      View Ops
                    </button>

                    <button className="py-2 rounded-lg border border-m3-outline-variant text-xs font-medium hover:bg-slate-100 transition-colors">
                      Dispatch
                    </button>
                  </div>
                </div>
            ...
            <div className="absolute left-4 bottom-4 bg-black/75 backdrop-blur-md text-white rounded-xl px-4 py-3 shadow-2xl border border-white/10 z-10">
              ...
              <div className="space-y-1 text-xs text-white/80">
                <p>• 4 Active Transfer Routes</p>
                <p>• 2 Critical Shortage Zones</p>
                <p>• 18 Regional Centers Connected</p>
              </div>
            </div>
          </div>
        </div>
