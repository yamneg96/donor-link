...
export default function DashboardPage() {
  ...
  const capacityPercent =
    stats?.capacity && stats.capacity > 0
      ? Math.round((stats.totalUnits / stats.capacity) * 100)
      : 75;
  ...
  const inventoryByType = stats?.byType ?? {};
      ...
      <div className="flex justify-between items-end mb-stack-lg">
        <div>
          ...
          <p className="font-body-lg text-body-lg text-on-surface-variant mt-2">
            Live operational overview for City General.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-gutter">
        ...
        <div className="col-span-1 md:col-span-8 bg-white rounded-xl card-shadow p-6 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="space-y-3">
            {requestsLoading ? (
              <div className="text-on-surface-variant p-4 text-center">
              ...
              <div className="flex justify-between items-center p-3 bg-error-container/20 rounded-lg border border-error-container/50">
                ...
                <button className="px-4 py-2 bg-error text-on-error rounded-lg font-label-md text-label-md hover:bg-error/90 transition-colors">
                  Fulfill
                </button>
              </div>
            )}
        ...
        <div className="col-span-1 md:col-span-4 bg-white rounded-xl card-shadow p-6 flex flex-col items-center justify-center text-center">
          <h3 className="font-headline-md text-headline-md text-on-surface w-full text-left mb-6">
            Facility Capacity
          </h3>
          ...
          <p className="font-body-md text-body-md text-on-surface-variant">
            {stats?.totalUnits && stats?.capacity
              ? `${stats.totalUnits} / ${stats.capacity} Units Available`
              : '420 / 560 Units Available'}
          </p>
        </div>
        ...
        <div className="col-span-1 md:col-span-12 bg-white rounded-xl card-shadow p-6 mt-stack-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            {bloodTypes.map((type) => {
              const count =
                inventoryByType[type] ?? fallbackCounts[type] ?? 0;
              ...
              return (
                <div
                  key={type}
                  className={`rounded-lg p-4 flex flex-col items-center border-b-2 ${
                    isLow
                ...
                >
                  ...
                  <span
                    className={`font-display-lg text-display-lg leading-none mb-2 ${
                      isLow
                        ? 'text-error'
                        : type === 'O+'
                        ? 'text-primary'
